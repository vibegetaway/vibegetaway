...
interface MemoizedMarkerProps {
  ...
  onHover: (dest: Destination, e: L.LeafletMouseEvent) => void
  onLeave: () => void
}

const MemoizedMarker = memo(({ dest, isSelected, onClick, onHover, onLeave }: MemoizedMarkerProps) => {
  const hasDetails = !!(dest.description && dest.pricing)

  const icon = useMemo(() => createPurpleIcon(isSelected, hasDetails), [isSelected, hasDetails])

  const position: [number, number] = useMemo(() =>
    dest.coordinates
      ? [dest.coordinates.lat, dest.coordinates.lng]
      : COUNTRY_CENTROIDS[dest.country] || [0, 0]
  , [dest.coordinates, dest.country])
