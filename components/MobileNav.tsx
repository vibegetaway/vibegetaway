...
export function MobileNav({ activePage, hidePlanButton = false }: MobileNavProps) {
  ...
  return (
    <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-background/95 backdrop-blur-xl border-t border-border/50 safe-area-inset-bottom shadow-lg">
      <div className="absolute inset-x-0 top-0 h-0.5 bg-gradient-to-r from-primary/40 via-secondary/40 to-primary/40" />
          ...
          {isSignedIn ? (
            <div className="flex flex-col items-center gap-1">
              <UserButton
                appearance={{
                  elements: {
                    avatarBox: "w-10 h-10 rounded-full ring-2 ring-border/50 transition-all shadow-sm hover:ring-primary/50 hover:scale-110 active:scale-95",
